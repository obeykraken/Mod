...
import net.mcreator.futureshell.world.biome.NeonaturaBiome;
...
public class FutureShellModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, FutureShellMod.MODID);
	public static final RegistryObject<Biome> NEONATURA = REGISTRY.register("neonatura", () -> NeonaturaBiome.createBiome());
		...
		for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
			...
			if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					parameters.add(new Pair<>(NeonaturaBiome.PARAMETER_POINT,
							biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, NEONATURA.getId()))));
				...
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					...
					if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
						...
						surfaceRules.add(1, preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, NEONATURA.getId()),
								FutureShellModBlocks.NEOHIERBA.get().defaultBlockState(), FutureShellModBlocks.NEOTIERRA.get().defaultBlockState(),
								FutureShellModBlocks.NEOTIERRA.get().defaultBlockState()));
